...
class Stabilogram:
    ...
    def from_array(
        ...
        if n_columns == 2:
            assert original_frequency is not None or time is not None, (
                "Need to provide a frequency for the signal (parameter original frequency), or timestamps"
            )
        ...
        if resample:
            ...
            assert original_frequency is not None, (
                "Need to provide a frequency for the signal (parameter original frequency) when resample is set to False"
            )
    ...
    def _compute_power_spectrum(self) -> None:
        ...
        freqs, psd = welch(
            ...
            fs=self.frequency,
            detrend="linear",
            nperseg=10 * self.frequency,
            noverlap=0.5 * 10 * self.frequency,
            axis=0,
    ...
    def _compute_diffusion_plot(self, duration_ratio=1 / 3) -> None:
        ...
        msd = [np.array([0, 0])] + [
            np.mean((self.signal[i:, :] - self.signal[: (n - i), :]) ** 2, axis=0) for i in range(1, max_ind + 1)
        ]
    ...
    def _compute_speed(self, window_length=5, polyorder=3) -> None:
        ...
        """
        cop = self.signal
        spd_savgol = savgol_filter(
            x=cop, window_length=window_length, polyorder=polyorder, deriv=1, axis=0, delta=1 / self.frequency
        )
    ...
    def get_signal(self, name, **kwargs) -> np.ndarray:
        ...
        if name == labels.DIFF_MLAP:
            return self.diffusion_plot[:, 0], self.diffusion_plot[:, 1] + self.diffusion_plot[
                :, 2
            ]  # is it a sum really ?
